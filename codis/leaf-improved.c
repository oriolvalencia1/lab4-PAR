...
#ifndef ROWS
#define ROWS (16*64)             /* size of display window in pixels */
#endif

#ifndef COLS
#define COLS (16*64)             /* size of display window in pixels */
#endif
...
int pixel_dwell(int width, int height, double CminR, double CminI, double CmaxR, double CmaxI, int x, int y, double scale_real, double scale_imag, int maxiter)
{
      int dwell = 0;
...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter, int d)
{
		...
		#pragma omp taskgroup
		{
			if (NRows > TILE) {
				if (d < CUTOFF) {
					...
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					#pragma omp task firstprivate(NRows, NCols, start_fil, start_col, d)
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil + NRows / 2, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					#pragma omp task firstprivate(NRows, NCols, start_fil, start_col, d)
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					#pragma omp task firstprivate(NRows, NCols, start_fil, start_col, d)
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil + NRows / 2, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
				} else {
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil + NRows / 2, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					mandel_tiled_rec(M, NRows / 2, NCols / 2, start_fil, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
				...
				if (d < CUTOFF) {
					#pragma omp task firstprivate(NRows, NCols, start_fil, start_col, d)
					mandel_tiled_rec(M, NRows, NCols / 2, start_fil, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					#pragma omp task firstprivate(NRows, NCols, start_fil, start_col, d)
					mandel_tiled_rec(M, NRows, NCols / 2, start_fil, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
				} else {
					mandel_tiled_rec(M, NRows, NCols / 2, start_fil, start_col, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
					mandel_tiled_rec(M, NRows, NCols / 2, start_fil, start_col + NCols / 2, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, d + 1);
				}
...
void create_image(unsigned char *image, int matrix[ROWS][COLS], int height, int width, int maxiter)
{
...
void create_grid(unsigned char *image, int height, int width)
{
      int ind;
...
int main(int argc, char *argv[])
{
      ...
      int height = ROWS;
      double size = N, x0 = -1.81641011, y0= -0.203125011;
      int *Hmatrix;
      unsigned char *image;
      TRACE_START_MARK;
      ...
      for (int i = 1; i < argc; i++) {
	    if (strcmp(argv[i], "-d") == 0) {
		  ...
		  fprintf(stderr, "Usage: %s [-o -h -d -i maxiter -c x0 y0 -s size]\n", argv[0]);
		  fprintf(stderr, "       -o to write computed image (mandel_image.jpg) and histogram (mandel_histogram.out) to disk (default no file generated)\n");
		  ...
		  fprintf(stderr, "       -i to specify maximum number of iterations at each point (default 100)\n");
		  fprintf(stderr, "       -c to specify the center x0+iy0 of the square to compute\n");
      ...
      fprintf(stdout, "    center = (%g, %g) \n    size = %g\n    maximum iterations = %d\n", (real_max + real_min) / 2, (imag_max + imag_min) / 2, (real_max - real_min) / 2, maxiter);
      fprintf(stdout, "\n");
	  ...
	  #pragma omp parallel
	  #pragma omp single
      mandel_tiled_rec((int (*)[width])Hmatrix, height, width, 0, 0, real_min, imag_min, real_max, imag_max, scale_real, scale_imag, maxiter,0);
      ...
      if ((output2file) && (fp != NULL)) {
	    ...
	    create_image(image, (int (*)[width])Hmatrix, height, width, maxiter);
      ...
      if (output2display) {
	    ...
	    if (setup_return == EXIT_SUCCESS) {
		  interact(display, &win, width, height, real_min, real_max, imag_min, imag_max);
	    }
      }
      TRACE_END_MARK;
      return EXIT_SUCCESS;
