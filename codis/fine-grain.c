...
#ifndef ROWS
#define ROWS (64*16)             /* size of display window in pixels */
#endif

#ifndef COLS
#define COLS (64*16)             /* size of display window in pixels */
#endif
...
void mandel_tiled(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      for (int y = 0; y < ROWS; y += TILE)
		...
		{
			int eV,eH;
		// Fill in matrix positions with the same value
			#pragma omp taskgroup
			{
			// vertical borders: skip top/bottom rows (corners owned by eH) to avoid race
			#pragma omp task shared(eV)
			{
			eV = 1;
			int ref = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI, x, y, scale_real, scale_imag, maxiter);
			for (int py = y; py < y + TILE; py++) {
			      ...
			      eV = eV & (ref == M[py][x]);
			      eV = eV & (ref == M[py][x + TILE - 1]);
			}
			}
			// horizontal borders: full top and bottom rows (owns all 4 corners)
